...
use Kibb\Backup\Databases\MongoDb;
use Kibb\Backup\Databases\MySql;
use Kibb\Backup\Databases\Sqlite;
use Kibb\Backup\Exceptions\CannotCreateDbDumper;
    ...
    protected static function processExtraDumpParameters(array $dumpConfiguration, DbDumper $dbDumper): DbDumper
    {
        collect($dumpConfiguration)->each(function ($configValue, $configName) use ($dbDumper){
            ...
            if (method_exists($dbDumper, $methodName)){
                ...
                static::callMethodDumper($dbDumper, $methodName, $methodValue);
            }
        });
        return $dbDumper;
    }
    protected static function callMethodDumper(DbDumper $dbDumper, string  $methodName, $methodValue): DbDumper
    {
        if (! $methodValue){
            $dbDumper->$methodName();
            return $dbDumper;
